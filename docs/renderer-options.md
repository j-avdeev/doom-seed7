...
existing Task 5-7 JavaScript WAD bridge. It keeps the Seed7-generated WASM
framebuffer provider unchanged and does not add textures, sprites, enemies,
weapons, combat, sound, or a Doom BSP renderer.
...
- `First-person`: the Task 8 untextured wall prototype.
...
untextured wall column is projected using a fixed prototype wall height. Wall
color is distance shaded only. The upper half of the framebuffer is a flat
ceiling color and the lower half is a flat floor color.
...
does not sample patches, flats, palettes, sprites, or things.
...
- The Canvas shows flat ceiling and floor bands with untextured wall columns.
...
Task 9 is not implemented by this prototype. The next safe renderer milestone is
palette and wall texture work after this untextured projection path is verified.
